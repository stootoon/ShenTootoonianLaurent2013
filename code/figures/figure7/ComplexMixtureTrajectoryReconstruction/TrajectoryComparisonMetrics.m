...
switch (lower(whichMetric))
 ...
 case 'ssen'
  XcboMs = bsxfun(@minus, Xcbo, mean(Xcbo,1));
  M = reshape(sum((Xcbo - XrecCbo).^2,1)./sum(XcboMs.^2,1),numBins, numOdors);
 case 'corr'
  M = 1 - reshape(corrdist(reshape(Xcbo,numCells,[]), reshape(XrecCbo,numCells,[])),numBins, numOdors);
 case 'r2'
